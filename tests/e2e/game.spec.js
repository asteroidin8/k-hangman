...
test("opens how to play from the toolbar without stats or share actions", async ({ page }) => {
  ...
  await expect(page.locator("#howToPlay")).toContainText("힌트는 막힐 때 랜덤 자모를 알려줍니다.");
  await expect(page.locator("#howToPlay")).toContainText("tip. 추가 힌트를 얻을 수 있습니다.");
...
test("hides wrong jamo values but keeps the area until the setting is enabled", async ({ page }) => {
  await page.goto("/");
  ...
  await page.getByRole("button", { name: "ㅎ" }).click();
  await expect(page.locator("#wrongJamoList")).toContainText("ㅎ");
});
